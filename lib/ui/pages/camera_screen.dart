import 'dart:async';
import 'dart:io';
import 'package:edverhub_video_editor/providers/camera_providers.dart';
...
class _CameraScreenState extends State<CameraScreen> with WidgetsBindingObserver, TickerProviderStateMixin {
  ...
  void onVideoRecordButtonPressed(CameraTimer a) {
    a.startTimer();
  ...
  Future<void> onStopButtonPressed(CameraTimer a) async {
    a.cancelTimer();
  ...
  void onPauseButtonPressed(CameraTimer a) {
    a.cancelTimer();
  ...
  void onResumeButtonPressed(CameraTimer a) {
    a.startTimer();
  ...
  Widget build(BuildContext context) {
    var ctP = Provider.of<CameraTimer>(context);
    return Scaffold(
      ...
      body: Stack(
        children: [
                      ...
                      GestureDetector(
                        onTap: () {
                          ...
                          return controller != null && controller.value.isInitialized && !controller.value.isRecordingVideo
                              ? onVideoRecordButtonPressed(ctP)
                              : (controller.value.isRecordingPaused)
                                  ? ctP.totalTimeInSeconds < 30
                                      ? onResumeButtonPressed(ctP)
                                      : showInSnackBar('30 seconds Completed')
                                  : onPauseButtonPressed(ctP);
                        },
                        child: CircleAvatar(
                          radius: 30,
                      ...
                      onPressed: () async {
                        if (controller != null && controller.value.isInitialized && controller.value.isRecordingVideo) {
                          if (controller.value.isRecordingPaused) {
                            if (ctP.totalTimeInSeconds == 0) {
                              showInSnackBar('No clip to delete');
                            } else {
                              ctP.addToTrimTimes([ctP.totalTimeInSeconds - ctP.lasttime, ctP.totalTimeInSeconds]);
                              ctP.totalTimeInSeconds = ctP.totalTimeInSeconds - ctP.lasttime;
                              ctP.lasttime = ctP.totalAndLastTime[ctP.totalAndLastTime.length - 1][1];
                          ...
                          if (Platform.isAndroid) {
                            if (await _requestPermission(Permission.storage)) {
                              ...
                              if (await directory.exists()) {
                                ...
                                for (int i = ctP.trimtimes.length - 1; i >= 0; i--) {
                                  if (ctP.trimtimes[i][0] / 10 == 0) {
                                    startTime = "00:00:0${ctP.trimtimes[i][0]}";
                                  } else {
                                    startTime = "00:00:${ctP.trimtimes[i][0]}";
                                  }
                                  if (ctP.trimtimes[i][1] / 10 == 0) {
                                    endTime = "00:00:0${ctP.trimtimes[i][1]}";
                                  } else {
                                    endTime = "00:00:${ctP.trimtimes[i][1]}";
                                  }
            ...
            child: Padding(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  ...
                  if (controller != null && controller.value.isInitialized) {
                    if (controller.value.isRecordingVideo) {
                      return onStopButtonPressed(ctP);
                    }
                    if (controller.value.isRecordingPaused) {
                      return onStopButtonPressed(ctP);
                    }
          ...
          Padding(
            ...
            // ),
            child: Consumer<CameraTimer>(
              builder: (BuildContext context, ctProv, Widget child) {
                if (ctProv.onStopButtonPressed) {
                  onStopButtonPressed(ctProv);
                }
                return LinearProgressIndicator(
                  backgroundColor: iconsColor,
                  valueColor: AlwaysStoppedAnimation<Color>(Colors.orange),
                  value: ((ctProv.totalTimeInMilliseconds) / 30000).toDouble(),
                );
              },
            ),
          ),
